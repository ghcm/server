...
exports.get = function(req, res){
    ...
    var myDocument = Company.findOne({ name: companyName }, function(err, result) {
        ...
        if (result) {
            res.render('company/edit_company', { title: 'Express', company: result  });
        } else {
            // we don't
        }
    });
...
exports.post = function(req, res, next){
    ...
    if (!Company.schema.methods.validateObj(replaceObject)) {
        Company.findOne({ "_id": objectId}, function(err, result) {
            console.log(result);
            res.render('company/edit_company', { title: 'Express', error: "Please, check entered data", company: result });
        });
